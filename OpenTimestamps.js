...
module.exports = {
  ...
  info(file_ots) {
    ...
    const ots_bytes = Utils.hexToBytes(file_ots);
    ctx.open(ots_bytes);
  ...
  stamp(file) {
    ...
    const nonce_appended_stamp = file_timestamp.timestamp.ops.set(0, new Ops.OpAppend(random16));// <--? on generation
    const merkle_root = nonce_appended_stamp.ops.set(0, new Ops.OpSHA256());

        // merkle_tip = make_merkle_tree(merkle_roots)
    const merkle_tip = merkle_root;
  ...
  create_timestamp(timestamp, calendar_urls) {
    ...
    const n = calendar_urls.length; // =1
  ...
  verify(file_ots, file) {
    ...
    const detached_timestamp = DetachedTimestampFile.DetachedTimestampFile.deserialize(ctx);
